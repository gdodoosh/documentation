...
on pricelist forms provides the option to add specific products, and a specific price, to a
pricelist.

To add a specific product and price to a pricelist on a pricelist form, click the :guilabel:`Price
Rules` tab, then click :guilabel:`Add a line` in the :guilabel:`Products` column. Then, select or
type in the desired product for which a specific price should be applied.

Next, if necessary, select a specific product variant under the :guilabel:`Variants` column (e.g. a
specific product size, color, etc.). Under the :guilabel:`Min. Quantity` column, designate if there
should be a minimum amount of the product purchased, in order to apply the specific price.

To configure the price of this specific product, enter in the desired amount under the
:guilabel:`Price` column. Then, there is the option to add a :guilabel:`Start Date` and
:guilabel:`End Date` to the configured product price, if required.

To add another product line, click :guilabel:`Add a line` again, and repeat the process. There is no
limit to how many products can be added in the :guilabel:`Price Rules` tab of a pricelist form, with
the :guilabel:`Multiple prices per product` feature activated.
...
Time-based rules are typically used with subscription products. Be sure to check out the
documentation.
...
printed _salesproduct catalog is available.
...
:guilabel:`Customers` page, or by clicking on the customer's name on a _saleste reveals that customer's contact form.
...
.. note::
   ...
   it is also reflected on the pricelist itself. So, if that pricelist form is opened, after a price
   rule/pricelist is added to a product in this fashion, that product appears on the pricelist form.
...
On the :guilabel:`Pricelists` page, select the desired pricelist to edit. On the pricelist form,
click into the :guilabel:`Configuration` tab.

In the :guilabel:`Configuration` tab, at the bottom, a new :guilabel:`Discounts` section is
available, regardless of the chosen pricelist setting (:guilabel:`Multiple prices per product` or
:guilabel:`Advanced price rules (discounts, formulas)`, even if no discount has been configured yet.
